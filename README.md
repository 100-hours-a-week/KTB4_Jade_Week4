...
이를 통해 컨테이너의 헬스체크 통과는 요청을 받을 수 있다는 의미일 뿐, 실제 사용자 요청 경로의 성능까지 안정화되었다는 의미는 아니라는 점을 확인했습니다. 이후 배포 완료 조건에 애플리케이션 기동 여부뿐 아니라 주요 실행 경로의 워밍업 과정도 포함했습니다.
...
이 방식은 엔티티를 조회한 뒤 Java에서 값을 증가시키는 방식과 달리 DB에서 직접 카운터를 변경하므로, 여러 요청이 동시에 들어와도 증가 연산이 유실되지 않는다는 장점이 있습니다.

하지만 JPQL 벌크 연산은 영속성 컨텍스트를 거치지 않고 DB에 직접 실행됩니다. 따라서 DB의 `liked_count`는 변경되었지만, 같은 트랜잭션의 JPA 1차 캐시에 이미 저장되어 있던 `Article` 엔티티에는 변경 전 값이 그대로 남아 있었습니다.
...
하지만 해당 방법은 현재 트랜잭션에서 관리 중인 다른 엔티티까지 모두 준영속 상태로 전환합니다. 좋아요 수 하나를 최신화하기 위해 영속성 컨텍스트 전체를 비우는 것은 영향 범위가 크다고 판단했습니다.

`refresh()`를 이용해 게시글 엔티티를 다시 읽는 방법도 있지만, 응답에 필요한 값은 좋아요 수 하나뿐이므로 엔티티 전체를 다시 조회하는 것보다 필요한 컬럼만 조회하는 방식을 선택했습니다.

#### 개선 결과

DB에 반영된 좋아요 수와 API 응답의 좋아요 수가 일치하게 되었습니다.

동시에 카운터 갱신은 기존의 원자적 벌크 UPDATE 방식을 유지해 동시 요청에서 발생할 수 있는 유실 업데이트를 방지했습니다. 이를 통해 벌크 연산은 성능과 동시성 측면에서 유용하지만, 영속성 컨텍스트와 DB 상태가 자동으로 동기화되지 않는다는 점을 고려해야 한다는 것을 확인했습니다.
